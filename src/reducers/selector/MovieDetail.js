...
const selectMobileData = createSelector(
  ...
  (movieDetailReducer) => {
    ...
    const isEmptyData =
      movieDetailReducer.movieDetailShowtimes.heThongRapChieu?.length === 0;
    const heThongRapChieu =
      movieDetailReducer.movieDetailShowtimes.heThongRapChieu;
        ...
        return [
          ...
          ...heThongRapChieuItem.cumRapChieu?.reduce(
            (colect2, cumRapChieuItem) => {
              return [
                ...colect2,
                ...cumRapChieuItem.lichChieuPhim?.reduce(
                  (colect3, lichChieuPhimItem) => {
                    return [
                      ...colect3,
                      {
                        ...lichChieuPhimItem,
    ...
    const arrayDay = [
      ...new Set(
        arrayAllLichChieuPhimAddProp?.map((item) =>
    ...
    const arrayHeThongRapChieuFilterByDay = arrayDay.map((date) => {
      ...
      const arrayHeThongRapItem = arrayHeThongRapRemoveDup.map(
        (heThongRapItem) => {
          const arrayLichChieuPhimFilterByHeThongRap =
            arrayLichChieuPhimFilterByDay?.filter(
              (item) => item.tenHeThongRap === heThongRapItem.tenHeThongRap
            );
          const arrayCumRapChieuRemoveDup =
            arrayLichChieuPhimFilterByHeThongRap?.filter(
              (itemIncrease, indexIncrease, arr) =>
                indexIncrease ===
...
const selectCommentByMaPhimAndCommentTest = createSelector(
  (state, maPhim) =>
    state.movieDetailReducer.commentList.filter(
      (item) => item.dataTest || item.maPhim === maPhim
...
export {
  selectMobileData,
  selectDesktopData,
  selectCommentByMaPhimAndCommentTest,
};
